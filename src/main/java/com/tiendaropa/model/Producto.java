...
public class Producto {
    ...
    /**
     * Porcentaje de comision (0 a 100) que gana el vendedor por cada unidad de este
     * producto que se venda con un pedido asociado a el. Lo define el Jefe, porque es
     * quien decide que le conviene incentivar vender mas.
     */
    @Column(name = "comision_pct", nullable = false)
    private java.math.BigDecimal comisionPct = java.math.BigDecimal.ZERO;
    ...
    public java.math.BigDecimal getComisionPct() { return comisionPct; }
    public void setComisionPct(java.math.BigDecimal comisionPct) { this.comisionPct = comisionPct; }

    /** Cuanto gana el vendedor por "cantidad" unidades a "precioUnitario" (el congelado en el pedido). */
    public java.math.BigDecimal comisionDe(int cantidad, java.math.BigDecimal precioUnitario) {
        if (comisionPct == null || comisionPct.signum() <= 0) return java.math.BigDecimal.ZERO;
        return precioUnitario.multiply(java.math.BigDecimal.valueOf(cantidad))
                .multiply(comisionPct)
